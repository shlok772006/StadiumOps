...
/**
 * Build an emergency action plan prompt.
 */
function buildEmergencyPrompt(incidentType, location, details) {
...
/**
 * Build a crowd analysis prompt.
 */
function buildCrowdAnalysisPrompt() {
  const crowd = getLiveCrowdDensity();
  const weather = getWeatherData();
 ...
 */
function buildReportPrompt(reportType) {
  const crowd = getLiveCrowdDensity();
  const weather = getWeatherData();
  const t = TOURNAMENT_CONTEXT;
...
/**
 * Route a staff/operations query through the AI.
 */
